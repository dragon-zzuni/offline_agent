...
# -*- coding: utf-8 -*-
"""
CareBridge Integration 프로젝트 분류 수정 스크립트

CC로 잘못 분류된 CareBridge Integration 관련 TODO들을 CI 프로젝트로 재분류합니다.
"""
import sys
import os
sys.path.append('.')
...
def main():
    print("=== CareBridge Integration 프로젝트 분류 수정 ===\n")
    ...
    # CareBridge 관련이지만 잘못 분류된 TODO 찾기
    cur.execute('''
        SELECT id, title, description, project, source_message
        FROM todos 
        WHERE (title LIKE '%CareBridge%' OR description LIKE '%CareBridge%' OR source_message LIKE '%CareBridge%')
        AND project != 'CI'
    ''')
    
    misclassified_todos = cur.fetchall()
    print(f"잘못 분류된 CareBridge TODO: {len(misclassified_todos)}개")
